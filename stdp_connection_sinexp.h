...
private:
  ...
  // index
  void update_dopamine_( const std::vector< nest::spikecounter >& dopa_spikes,
    const STDPSinExpCommonProperties& cp );

  void update_weight_( double weight_change,
    const STDPSinExpCommonProperties& cp );

  void process_dopa_spikes_( const std::vector< nest::spikecounter >& dopa_spikes,
    double t0,
    double t1,
    const STDPSinExpCommonProperties& cp );
...
template < typename targetidentifierT >
STDPSinExpConnection< targetidentifierT >::STDPSinExpConnection()
  : ConnectionBase()
  , weight_( 1.0 )
  , dopa_spikes_idx_( 0 )
  , t_last_update_( 0.0 )
...
template < typename targetidentifierT >
STDPSinExpConnection< targetidentifierT >::STDPSinExpConnection(
  const STDPSinExpConnection& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
...
template < typename targetidentifierT >
void
STDPSinExpConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
inline void STDPSinExpConnection< targetidentifierT >::process_dopa_spikes_(
  const std::vector< nest::spikecounter >& dopa_spikes,
  double t0,
  double t1,
  const STDPSinExpCommonProperties& cp )
{
  ...
  if ( ( dopa_spikes.size() > dopa_spikes_idx_ )
    && ( dopa_spikes[ dopa_spikes_idx_ ].spike_time_ <= t1
    && dopa_spikes[ dopa_spikes_idx_+1 ].multiplicity_ == vt_num_) )
  {
...
template < typename targetidentifierT >
inline void
STDPSinExpConnection< targetidentifierT >::trigger_update_weight( nest::thread t,
  const std::vector< nest::spikecounter >& dopa_spikes,
  const double t_trig,
  const STDPSinExpCommonProperties& cp )
{
